 ...
 */
import { issueJobList, issueJobRemove, issueJobPause, issueJobPublish,  issueJobDelay} from '../constants/api';
...
function Page(props: Props) {
  ...
  const { tableProps, search }: any = useAntdTable(
    ...
    issueJobList({ timeType, jobCopsSign: 0, pageSize: paginatedParams.pageSize, pageNum: paginatedParams.current, ...tableProps }),
    {
      form,
  ...
  const onAdd = () => {
    history.push(`/upload/params/add`)
  }
  ...
  const onEdit = (item: any) => {
    ...
    history.push(`/upload/params/edit?id=${item.id}`)
  }

  const onRemove = (item: any) => {
    Modal.confirm({
      title: '提示',
      content: `确认要删除当前参数下发吗?`,
      okText: '确定',
      cancelText: '取消',
      onOk: async () => {
        try {
          ...
          setLoading(true);
          const result = await issueJobRemove(param);
          setLoading(false);
          invariant(result && result.code === RESPONSE_CODE.success, result && result.msg || '删除参数下发失败，请重试');
          notification.success({ message: '删除参数下发成功!' });
          customSubmit();
        } catch (error) {
  ...
  const onDetail = (item: any) => {
    history.push(`/upload/params/detail?id=${item.id}`)
  }
  ...
  const onCopy = () => {
    ...
    history.push(`/upload/params/copy?id=${selectedRowKeys[0]}&type=1`);
  }
  ...
  const onOperationDetail = () => {
    ...
    history.push(`/upload/params/operation?id=${selectedRowKeys[0]}&jobName=${selectedRows[0].jobName}`);
  }
  ...
  const onStart = async () => {
    ...
    setLoading(true);
    const res = await issueJobPublish(selectedRowKeys[0]);
    setLoading(false);
  ...
  const onPause = async () => {
    ...
    setLoading(true);
    const res = await issueJobPause(selectedRowKeys[0]);
    setLoading(false);
    if (res && res.code === RESPONSE_CODE.success) {
  ...
  const handleOk = async () => {
    ...
    setLoading(true);
    const res = await issueJobDelay(param);
    setLoading(false);
